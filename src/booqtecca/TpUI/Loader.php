...
class Loader extends PluginBase
{
    ...
    public function onEnable(): void
    {
        ...
        $this->getServer()->getCommandMap()->registerAll('TpUI', [
                new TpUICommands($this),
     ...
     */
    public function showTpUI(Player $player): void
    {
        $form = new SimpleForm(TextFormat::DARK_PURPLE . 'Warps', 'Click to teleport to a warp');
        foreach (self::getWarps() as $warp) {
            if ($player->hasPermission('TpUI.warp') || $player->hasPermission('TpUI.warp.*') || $player->hasPermission('TpUI.warp.' . TextFormat::clean(strtolower($warp)))) {
                $form->addButton(new Button($warp));
            }
        }
        $form->setCallable(static function (Player $player, $data) {
            if ($player->hasPermission('TpUI.warp') || $player->hasPermission('TpUI.warp.*') || $player->hasPermission('TpUI.warp.' . TextFormat::clean(strtolower($data)))) {
    ...
    public function showWorldUI(Player $player): void
    {
        ...
        foreach (self::getAllWorlds() as $world) {
            if ($player->hasPermission('TpUI.world') || $player->hasPermission('TpUI.world.*') || $player->hasPermission('TpUI.world.' . TextFormat::clean(strtolower($world)))) {
                $form->addButton(new Button($world));
            }
        }
        $form->setCallable(static function (Player $player, $data) {
            if ($player->hasPermission('TpUI.world') || $player->hasPermission('TpUI.world.*') || $player->hasPermission('TpUI.world.' . TextFormat::clean(strtolower($data)))) {
